...
def train(model, opt, args):
    ...
    for it in tqdm(range(args.steps)):
        ...
        while not all(dones):
            ...
            log_pbs = torch.zeros(args.batch_size)

            for i in range(args.batch_size):
                if torch.equal(next_states[i], env.get_fix_initial_state()[i]):
                    log_pbs[i] = torch.log(torch.tensor(1e-7))
                else:
                    log_pbs[i] = torch.log(torch.tensor([1/(args.p - 1)]))
